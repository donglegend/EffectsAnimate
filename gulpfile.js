...
var del = require('del');
var fs = require('fs');
var safeWriteFile = require('safe-file-write');
...
gulp.task('build', ['browserify', 'uglify'], function() {
  ...
  htmls.forEach(function(i, h) {
    ...
    output = output.replace(
      /src="\.\/static\/(js)\/[A-Z0-9a-z_]+\.js\"/g,
      function(word) {
        // console.log(word.replace(/\.js/, '.js' + '?' + new Date().getTime().toString(16)))
        return word
          .replace(/\js\//, 'js/min/')
          .replace(/\.js/, '.js' + '?' + new Date().getTime().toString(16));
      }
    );
    ...
    safeWriteFile.safeFileWriteSync(paths.dist + '/' + i, output);
  });
});
